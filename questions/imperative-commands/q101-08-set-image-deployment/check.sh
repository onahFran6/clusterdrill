...
check_criterion "Rollout completed - running pod's container also uses nginx:1.25-alpine" \
  bash -c "
    POD=\"\$(newest_pod_name '$QUESTION_ID' app=image-rollout)\"
    [ -n \"\$POD\" ] || exit 1
    kubectl get pod \"\$POD\" -n '$QUESTION_ID' -o jsonpath='{.spec.containers[0].image}' 2>/dev/null | grep -q '^nginx:1.25-alpine$'
  "
